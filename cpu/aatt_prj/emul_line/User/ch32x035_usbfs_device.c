...
void USBFS_Device_Endp_Init( void )
{
    ...
    USBFSD->UEP2_DMA = (uint32_t)(uint8_t *)&USB_Rx_Buf[ 0 ];
...
volatile uint8_t tst=0;
volatile uint8_t ok_tst=0;
            ...
            case USBFS_UIS_TOKEN_OUT:
                switch ( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP2:
                    {
                        ...
                        t_len= USBFSD->RX_LEN;
                        USB_Rx_PackLen[USB_Rx_LoadNum] = t_len;
                        if(t_len>MAX_RX_BUF_LEN)
                            t_len=MAX_RX_BUF_LEN;
                        if(t_len)
                          {
                          for(ii=0;ii<t_len;ii++)
                             {
                              usb_rx_b[ii]=  USB_Rx_Buf[ USB_Rx_LoadNum * DEF_USB_FS_PACK_LEN +ii];
                             }
                          usb_rx_len=t_len;
                          }
                        USB_Rx_LoadNum++;
                        USBFSD->UEP2_DMA = (uint32_t)(uint8_t *)&USB_Rx_Buf[ ( USB_Rx_LoadNum * DEF_USB_FS_PACK_LEN ) ];
#if 1
                        if(USB_Rx_LoadNum >= DEF_RX_BUF_NUM_MAX )
                        {
                            USB_Rx_LoadNum = 0x00;
                            USBFSD->UEP2_DMA = (uint32_t)(uint8_t *)&USB_Rx_Buf[ 0 ];
                        }
                        USB_Rx_DealNum++;
                        if( USB_Rx_RemainNum >= ( DEF_RX_BUF_NUM_MAX - 2 ) )
                        {
